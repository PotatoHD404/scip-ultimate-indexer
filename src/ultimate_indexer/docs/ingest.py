...
def _chunk_to_record(
    ...
    return ChunkRecord(
        ...
        chunk_id=chunk.chunk_id or sha256(
...
def _create_doc_symbols(
    ...
    doc_symbol = SymbolRecord(
        ...
        snippet=file_record.content[:500] if len(file_record.content) > 500 else file_record.content,
    ...
    for idx, section in enumerate(sections):
        ...
        section_symbol = SymbolRecord(
            ...
            kind="Section",
            relative_path=file_path,
            start_line=(header.line_number + 1) if header else section.get('start_line', 0),
            end_line=section.get('end_line', section.get('start_line', 0) + 10),
            signature=header.text if header else (anchor or f"Section {idx}"),
            docstring=section.get('content', '')[:200] if section.get('content') else '',
